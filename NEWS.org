...
* Version 2.16 (2026-09-13)

- Improve completion field handling in =corfu--in-region=.

* Version 2.15 (2026-09-03)

- Minor optimizations during candidate computation.
- Do not remove a file from the candidate list if it matches the input exactly,
  even if it should be filtered according to =completion-ignored-extensions=.

* Version 2.14 (2026-08-21)

- =corfu-popupinfo=: Bugfixes.

* Version 2.13 (2026-08-13)

- =corfu-popupinfo=: Bugfix when =corfu-popupinfo-delay= is set to nil.

* Version 2.12 (2026-08-01)

- =corfu-mouse=, =corfu-popupinfo=: Improve mouse scrolling, bugfixes.

* Version 2.11 (2026-07-22)
...
* Version 2.7 (2025-12-28)

- ~corfu-popupinfo-hide~: Change default to nil.
- ~corfu-popupinfo-delay~: Reduce delay when changing candidates to 0.5.
...
- Bugfixes.
- Add name to Corfu child frame to improve readability of =(frame-list)=.
...
- =corfu-auto-trigger=: New customization variable to trigger immediate completion
  after certain characters. See also =cape-capf-trigger=.
...
- Improve compatibility with Eglot. Fix a manual completion bug in the case when
  Eglot provided no matching candidates.
- Experimental: Use new function =set-frame-position-and-size= if available.
